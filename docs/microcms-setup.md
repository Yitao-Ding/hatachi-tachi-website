...
お知らせ本文の画像は、microCMS にアップロードしたもの (images.microcms-assets.io) だけを使う。
他サイトの画像 URL を直接貼ると、サイトの CSP (docs/security.md) の許可リストに無いため
Report-Only では Console に違反が出て、将来ブロックに切り替えた時に表示されなくなる。
